...
angular.module('impactApp')
  .controller('DemandePrestationsCtrl', function($state, RequestResource, prestations, demande) {
    ...
    this.submit = () => {
      ...
      RequestResource.update(this.demande).$promise.then(result => {
        this.demande = result;
        $state.go('demande');
      });
    };
    ...
    if (this.demande.data.prestations && this.demande.data.prestations.length > 0) {
      this.demande.data.prestations.forEach(prestation => {
